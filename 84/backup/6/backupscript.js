...
function clickPlayButton() {
    ...
    document.addEventListener("click", (e) => {
        ...
        currentIndex = songs.findIndex(item => item.song === src);
...
const removeLibrarySong = () => {
    if(librarySongs.findIndex(item => item.title == audioBarSongTitle.textContent) !== -1){
        const deleteLibrarySong = document.querySelector(`.library-card[data-audio="${songs[currentIndex]['song']}"]`);
        if(deleteLibrarySong) 
        {
            deleteLibrarySong.remove();
            librarySongs.splice(currentIndex,1);
            if(currentIndex!=0){
                currentIndex--;
            }
        }
