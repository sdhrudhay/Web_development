...
function changeLocation(){
    ...
    mq836.addEventListener('change', handleAudioBarOnResize);
...
async function loadTrendingJson() {
    ...
    console.log(songs);
    await showTrendingSongs(songs);
    clickTrendingPlayButton();
}
async function showTrendingSongs(songs){
    for(const item of songs){
        trendingCards.insertAdjacentHTML("beforeend",`
            <div class="card">
                    <div class="song-image">
                        <img src="images/${item['image']}" alt="">
                        <button type="button" class="play-button" data-audio="${item['song']}" data-index="${item['index']}"><img src="assests/play.svg" alt="" class="play-logo"></button>
                </div>
...
function clickTrendingPlayButton(){
    let currentSrc = null;
    let src = null;
...
function handleAudioBar(){
    ...
    playButton.addEventListener("click", ()=> {
        ...
        console.log(playPauseImage.src);
    });
    ...
    window.addEventListener('resize', () => {
        if(audioPlayerContainer.style.display != 'none' && window.innerWidth <=836){
            audioPlayerContainer.style.display = 'flex';
        }
        else if(audioPlayerContainer.style.display != 'none'){
            audioPlayerContainer.style.display = 'grid';
        }
    });
}
...
const mq836 = window.matchMedia("(max-width: 836px)");
